...
- Best Shot detection that measures the photos themselves — sharpness, exposure, faces and noise — so every group has a sensible default to keep.
- Optional one-tap enhancement for the photo you keep, with the original always one tap away.
...
- One-tap enhancement, always reversible
...
- Вибір найкращого знімка за самими фотографіями — різкістю, експозицією, обличчями й шумом, тож у кожній групі є розумний варіант залишити.
- Необовʼязкове покращення одним дотиком для знімка, який ви залишаєте, — оригінал завжди за один дотик.
...
- Покращення одним дотиком, яке завжди можна скасувати
...
- Verbesserung mit einem Tippen, immer umkehrbar
...
- Amélioration en une touche, toujours réversible
...
- Detección de la mejor toma medida en las propias fotos — nitidez, exposición, rostros y ruido —, para que cada grupo tenga una opción razonable que conservar.
- Mejora opcional con un toque para la foto que conservas, con el original siempre a un toque.
...
- Mejora con un toque, siempre reversible
...
- Detecção da melhor foto medida nas próprias fotos — nitidez, exposição, rostos e ruído —, para que cada grupo já tenha uma escolha sensata para manter.
- Aprimoramento opcional com um toque para a foto que você mantém, com o original sempre a um toque.
...
- Aprimoramento com um toque, sempre reversível
...
- Miglioramento con un tocco, sempre reversibile
...
- Herkenning van de beste opname, gemeten aan de foto's zelf — scherpte, belichting, gezichten en ruis — zodat elke groep een verstandige keuze klaar heeft staan.
- Optionele verbetering met één tik voor de foto die je houdt, het origineel altijd één tik verderop.
...
- Verbetering met één tik, altijd terug te draaien
...
- Wykrywanie najlepszego ujęcia mierzone na samych zdjęciach — ostrość, ekspozycja, twarze i szum — więc każda grupa ma sensowny domyślny wybór do zachowania.
- Opcjonalna poprawa jednym dotknięciem dla zdjęcia, które zostawiasz; oryginał zawsze o jedno dotknięcie dalej.
...
- Poprawa jednym dotknięciem, zawsze odwracalna
...
- Fotoğrafların kendisini ölçen En İyi Kare algılama — netlik, pozlama, yüzler ve gürültü — böylece her grupta saklanacak makul bir seçenek hazır bekler.
- Sakladığın fotoğraf için isteğe bağlı tek dokunuşluk iyileştirme; orijinali her zaman bir dokunuş uzakta.
...
- Tek dokunuşla iyileştirme, her zaman geri alınabilir
...
- 最佳照片偵測會實際衡量照片本身——清晰度、曝光、人臉與雜訊——讓每一組都有一個合理的保留預設值。
- 為你保留的那張照片提供選用的一鍵優化，原始照片永遠只差一次點按。
...
- كشف أفضل لقطة بقياس الصور نفسها — الحدة والإضاءة والوجوه والضوضاء — فلكل مجموعة خيار افتراضي معقول للاحتفاظ به.
- تحسين اختياري بلمسة واحدة للصورة التي تحتفظ بها، والأصل دائمًا على بُعد لمسة.
...
- تحسين بلمسة واحدة، يمكن التراجع عنه دائمًا
...
METADATA = {
    "en-US": {
        ...
        "release_notes": "If Alike ever closes unexpectedly, you can now tell me about it.\n\n- On the next calm launch, Alike asks once: “Alike closed unexpectedly” — Send Report or Not Now.\n- Send Report opens an ordinary email with the crash report attached. You read it before anything is sent, and you send it yourself.\n- The report is the crash diagnostic iOS recorded: the stack trace, the app and iOS versions, the device model and, for some crashes, the error message. Alike sends it as it is, and it contains no photos.\n- Not Now is final: Alike never asks about the same crash again.\n\nEverything still runs on your device: no account, no uploads, and Alike itself sends nothing — the email leaves only from your own mail app.\n\nFeedback and bug reports are genuinely welcome — the support link on the App Store page reaches me directly.",
    },
    "uk": {
        ...
        "release_notes": "Якщо Alike колись несподівано закриється, тепер про це можна мені розповісти.\n\n- Під час наступного спокійного запуску Alike один раз запитає: «Alike несподівано закрився» — «Надіслати звіт» або «Не зараз».\n- «Надіслати звіт» відкриває звичайний лист зі звітом про збій у вкладенні. Ви бачите його до надсилання й надсилаєте самі.\n- Звіт — це діагностика збою, яку записала iOS: стек викликів, версії застосунку та iOS, модель пристрою і, для деяких збоїв, текст повідомлення про помилку. Alike надсилає його як є, і фото в ньому немає.\n- «Не зараз» — остаточно: про той самий збій Alike більше не запитає.\n\nУсе так само виконується на вашому пристрої: без облікового запису й без вивантаження, а сам Alike нічого не надсилає — лист іде лише з вашого поштового застосунку.\n\nВідгуки та повідомлення про помилки дуже вітаються — посилання на підтримку на сторінці App Store веде безпосередньо до розробника.",
    },
    ...
    "de-DE": {
        ...
        "release_notes": "Falls Alike einmal unerwartet beendet wird, kannst du mir jetzt davon erzählen.\n\n- Beim nächsten ruhigen Start fragt Alike ein einziges Mal: „Alike wurde unerwartet beendet“ — „Bericht senden“ oder „Nicht jetzt“.\n- „Bericht senden“ öffnet eine gewöhnliche E-Mail mit dem Absturzbericht im Anhang. Du liest sie, bevor etwas gesendet wird, und sendest sie selbst.\n- Der Bericht ist die Absturzdiagnose, die iOS aufgezeichnet hat: der Stacktrace, die App- und iOS-Version, das Gerätemodell und bei manchen Abstürzen die Fehlermeldung. Alike sendet ihn unverändert, und er enthält keine Fotos.\n- „Nicht jetzt“ gilt endgültig: Zum selben Absturz fragt Alike nie wieder.\n\nAlles läuft weiterhin auf deinem Gerät: kein Konto, keine Uploads, und Alike selbst sendet nichts — die E-Mail verlässt das Gerät nur über deine eigene Mail-App.\n\nRückmeldungen und Fehlerberichte sind ausdrücklich willkommen — der Support-Link auf der App-Store-Seite erreicht mich direkt.",
    },
    "fr-FR": {
        ...
        "release_notes": "Si Alike se ferme un jour de façon inattendue, vous pouvez désormais me le signaler.\n\n- Au prochain lancement tranquille, Alike pose la question une seule fois : « Alike s’est fermé de façon inattendue » — « Envoyer le rapport » ou « Plus tard ».\n- « Envoyer le rapport » ouvre un e-mail ordinaire avec le rapport de plantage en pièce jointe. Vous le lisez avant tout envoi, et c’est vous qui l’envoyez.\n- Le rapport est le diagnostic du plantage enregistré par iOS : la trace, les versions de l’app et d’iOS, le modèle de l’appareil et, pour certains plantages, le message d’erreur. Alike l’envoie tel quel, et il ne contient aucune photo.\n- « Plus tard » est définitif : Alike ne redemande jamais pour le même plantage.\n\nTout se passe toujours sur votre appareil : aucun compte, aucun envoi, et Alike lui-même n’envoie rien — l’e-mail ne part que depuis votre propre app de messagerie.\n\nVos retours et vos rapports de bugs sont sincèrement bienvenus — le lien d'assistance sur la page App Store me parvient directement.",
    },
    "es-ES": {
        ...
        "release_notes": "Si Alike alguna vez se cierra inesperadamente, ahora puedes contármelo.\n\n- En el siguiente arranque tranquilo, Alike pregunta una sola vez: «Alike se ha cerrado inesperadamente»: «Enviar informe» o «Ahora no».\n- «Enviar informe» abre un correo normal con el informe del fallo adjunto. Lo lees antes de que se envíe nada y lo envías tú.\n- El informe es el diagnóstico del fallo que registró iOS: la traza, las versiones de la app y de iOS, el modelo del dispositivo y, en algunos fallos, el mensaje de error. Alike lo envía tal cual y no contiene fotos.\n- «Ahora no» es definitivo: Alike no vuelve a preguntar por el mismo fallo.\n\nTodo sigue ejecutándose en tu dispositivo: sin cuenta, sin subidas, y Alike por sí mismo no envía nada; el correo sale solo desde tu propia app de correo.\n\nLos comentarios y los informes de errores son muy bienvenidos: el enlace de soporte de la página de App Store llega directamente a mí.",
    },
    "es-MX": {
        "subtitle": "Encuentra y limpia duplicados",
    ...
    "pt-BR": {
        ...
        "release_notes": "Se o Alike fechar inesperadamente, agora você pode me contar.\n\n- Na próxima abertura tranquila, o Alike pergunta uma única vez: «O Alike fechou inesperadamente» — «Enviar relatório» ou «Agora não».\n- «Enviar relatório» abre um e-mail comum com o relatório da falha em anexo. Você lê antes de qualquer envio, e quem envia é você.\n- O relatório é o diagnóstico da falha registrado pelo iOS: o rastreamento, as versões do app e do iOS, o modelo do dispositivo e, em algumas falhas, a mensagem de erro. O Alike o envia como está, e ele não contém fotos.\n- «Agora não» é definitivo: o Alike nunca pergunta de novo sobre a mesma falha.\n\nTudo continua rodando no seu dispositivo: sem conta, sem envios, e o próprio Alike não envia nada — o e-mail só sai pelo seu app de e-mail.\n\nComentários e relatos de erros são muito bem-vindos — o link de suporte na página da App Store chega direto a mim.",
    },
    ...
    "it": {
        ...
        "release_notes": "Se Alike si chiude in modo imprevisto, ora puoi dirmelo.\n\n- Al successivo avvio tranquillo, Alike chiede una sola volta: «Alike si è chiuso in modo imprevisto» — «Invia report» o «Non ora».\n- «Invia report» apre una normale email con il report dell'arresto in allegato. La leggi prima che parta qualsiasi cosa, e la invii tu.\n- Il report è la diagnostica dell'arresto registrata da iOS: lo stack trace, le versioni dell'app e di iOS, il modello del dispositivo e, per alcuni arresti, il messaggio di errore. Alike lo invia così com'è, e non contiene foto.\n- «Non ora» è definitivo: Alike non chiede mai più dello stesso arresto.\n\nTutto avviene ancora sul tuo dispositivo: nessun account, nessun caricamento, e Alike da solo non invia nulla — l'email parte solo dalla tua app di posta.\n\nCommenti e segnalazioni sono davvero benvenuti: il link di assistenza sulla pagina App Store arriva direttamente a me.",
    },
    "nl-NL": {
        ...
        "release_notes": "Als Alike ooit onverwacht stopt, kun je me dat nu laten weten.\n\n- Bij de volgende rustige start vraagt Alike het één keer: 'Alike is onverwacht gestopt' — 'Stuur rapport' of 'Niet nu'.\n- 'Stuur rapport' opent een gewone e-mail met het crashrapport als bijlage. Je leest hem voordat er iets verstuurd wordt, en je verstuurt hem zelf.\n- Het rapport is de crashdiagnose die iOS heeft vastgelegd: de stacktrace, de app- en iOS-versie, het apparaatmodel en bij sommige crashes de foutmelding. Alike verstuurt het ongewijzigd, en er staan geen foto's in.\n- 'Niet nu' is definitief: over dezelfde crash vraagt Alike het nooit opnieuw.\n\nAlles draait nog steeds op je apparaat: geen account, geen uploads, en Alike zelf verstuurt niets — de e-mail vertrekt alleen vanuit je eigen mailapp.\n\nReacties en foutmeldingen zijn oprecht welkom — de ondersteuningslink op de App Store-pagina komt rechtstreeks bij mij terecht.",
    },
    "pl": {
        ...
        "release_notes": "Jeśli Alike kiedyś nieoczekiwanie się zamknie, możesz mi teraz o tym powiedzieć.\n\n- Przy następnym spokojnym uruchomieniu Alike zapyta jeden raz: „Aplikacja Alike nieoczekiwanie się zamknęła” — „Wyślij raport” lub „Nie teraz”.\n- „Wyślij raport” otwiera zwykłego e-maila z raportem o awarii w załączniku. Czytasz go, zanim cokolwiek zostanie wysłane, i wysyłasz go sam.\n- Raport to diagnostyka awarii zapisana przez iOS: ślad stosu, wersje aplikacji i iOS, model urządzenia oraz, przy niektórych awariach, komunikat o błędzie. Alike wysyła go bez zmian i nie zawiera on zdjęć.\n- „Nie teraz” jest ostateczne: o tę samą awarię Alike już nigdy nie zapyta.\n\nWszystko nadal działa na Twoim urządzeniu: bez konta, bez wysyłania czegokolwiek, a samo Alike niczego nie wysyła — e-mail wychodzi wyłącznie z Twojej aplikacji pocztowej.\n\nUwagi i zgłoszenia błędów są naprawdę mile widziane — link do pomocy na stronie App Store trafia bezpośrednio do mnie.",
    },
    "tr": {
        ...
        "release_notes": "Alike bir gün beklenmedik şekilde kapanırsa, artık bunu bana iletebilirsin.\n\n- Bir sonraki sakin açılışta Alike yalnızca bir kez sorar: \"Alike beklenmedik şekilde kapandı\" — \"Raporu Gönder\" ya da \"Şimdi Değil\".\n- \"Raporu Gönder\", çökme raporu ekli sıradan bir e-posta açar. Bir şey gönderilmeden önce okursun ve kendin gönderirsin.\n- Rapor, iOS'un kaydettiği çökme tanılamasıdır: yığın izi, uygulama ve iOS sürümleri, cihaz modeli ve bazı çökmelerde hata mesajı. Alike onu olduğu gibi gönderir; fotoğraf içermez.\n- \"Şimdi Değil\" kesindir: Alike aynı çökmeyi bir daha asla sormaz.\n\nHer şey yine cihazında çalışıyor: hesap yok, yükleme yok ve Alike kendisi hiçbir şey göndermez — e-posta yalnızca kendi posta uygulamandan çıkar.\n\nGeri bildirimler ve hata bildirimleri gerçekten memnuniyetle karşılanır — App Store sayfasındaki destek bağlantısı doğrudan bana ulaşır.",
    },
    "zh-Hant": {
        ...
        "release_notes": "如果 Alike 意外關閉，現在你可以告訴我。\n\n- 下次安穩啟動時，Alike 只會問一次：「Alike 意外關閉」——「傳送報告」或「現在不要」。\n- 「傳送報告」會開啟一封普通的電子郵件，附上當機報告。在寄出任何東西之前你都能先看過，並由你自己寄出。\n- 報告是 iOS 記錄的當機診斷：堆疊追蹤、App 與 iOS 版本、裝置型號，部分當機還包含錯誤訊息。Alike 會原封不動地寄出，報告不含照片。\n- 「現在不要」就是最終決定：同一次當機，Alike 不會再問。\n\n一切仍在你的裝置上執行：沒有帳戶、不上傳任何東西，Alike 本身也不傳送任何資料——郵件只會從你自己的郵件 App 寄出。\n\n歡迎提供意見與回報問題——App Store 頁面上的支援連結會直接寄到我這裡。",
    },
    "ar-SA": {
        ...
        "release_notes": "إذا أُغلق Alike يومًا بشكل غير متوقع، يمكنك الآن إخباري بذلك.\n\n- عند التشغيل الهادئ التالي، يسأل Alike مرة واحدة فقط: «تم إغلاق Alike بشكل غير متوقع» — «إرسال التقرير» أو «ليس الآن».\n- «إرسال التقرير» يفتح رسالة بريد عادية مرفقًا بها تقرير التعطل. تقرؤها قبل إرسال أي شيء، وأنت من يرسلها.\n- التقرير هو تشخيص التعطل الذي سجّله iOS: تتبّع المكدّس وإصدارا التطبيق وiOS وطراز الجهاز، وفي بعض حالات التعطل رسالة الخطأ. يرسله Alike كما هو، ولا يحتوي على صور.\n- «ليس الآن» قرار نهائي: لن يسأل Alike عن التعطل نفسه مرة أخرى.\n\nكل شيء ما زال يجري على جهازك: لا حساب ولا رفع، وAlike نفسه لا يرسل شيئًا — تخرج الرسالة من تطبيق البريد الخاص بك فقط.\n\nملاحظاتكم وبلاغاتكم مرحَّب بها فعلًا — رابط الدعم في صفحة App Store يصلني مباشرة.",
    },
}
