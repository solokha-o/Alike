...
final class CleanupHistorySnapshotTests: XCTestCase {
    ...
    func testMonthsAreCutOnTheCalendarTheHeaderIsSpelledIn() throws {
        ...
        hijri.timeZone = Calendar.alikeFormattingCalendar.timeZone
        ...
        let firstOfMarch = date(year: 2026, month: 3, day: 1)
        let hijriMonthOfFirst = hijri.component(.month, from: firstOfMarch)
        let secondDay = try XCTUnwrap(
            (2...31).first { day in
                guard let candidate = calendar.date(from: DateComponents(year: 2026, month: 3, day: day)) else {
                    return false
                }
        ...
        XCTAssertEqual(CleanupHistorySnapshot.groupingCalendar, Calendar.alikeFormattingCalendar)
