...
| `External/emilkowalski-skills/improve-animations` | Read-only animation audit and self-contained improvement plans | Pair with local `swiftui-performance-audit` or `swiftui-ui-patterns`; do not let web-specific rules override native platform behavior | No |
