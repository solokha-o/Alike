...
enum CompiledCatalogFixture {
    ...
    /// built `Purchases_PurchasesUI.bundle`.
    ...
    private static func compile(file: StaticString) throws -> URL {
        ...
        let root = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("PurchasesUILocalization-\(UUID().uuidString)")
    ...
    private static func infoPlist() throws -> Data {
        try PropertyListSerialization.data(
            fromPropertyList: [
                "CFBundleIdentifier": "com.alike.tests.purchasesui.localization",
